...
function Zlibrary:onDispatcherRegisterActions()
    Dispatcher:registerAction("zlibrary_search", {
        category = "none",
        event = "ZlibrarySearch",
        ...
        menu_items.zlibrary_main = {
            ...
            sub_item_table = {{
                ...
                sub_item_table = {{
                    ...
                    keep_menu_open = true,
                    callback = function()
                        Ui.showGenericInputDialog(T("Set base URL"), Config.SETTINGS_BASE_URL_KEY, Config.getBaseUrl(),
                            false, function(input_value)
                    ...
                    keep_menu_open = true,
                    callback = function()
                        Ui.showGenericInputDialog(T("Set email"), Config.SETTINGS_USERNAME_KEY,
                            Config.getSetting(Config.SETTINGS_USERNAME_KEY), false)
                    end
                }, {
                    text = T("Set password"),
                    keep_menu_open = true,
                    callback = function()
                        Ui.showGenericInputDialog(T("Set password"), Config.SETTINGS_PASSWORD_KEY,
                            Config.getSetting(Config.SETTINGS_PASSWORD_KEY), true)
                    end
                }, {
...
function Zlibrary:_fetchBookList(options)
    ...
    local function attemptFetch(retry_on_auth_error)
        ...
        local on_success = function(api_result)
            ...
            Ui.closeMessage(loading_msg)
            logger.info(string.format("Zlibrary:%s - Fetch successful. Results: %d", options.log_context,
                #api_result.books))
...
function Zlibrary:onSelectRecommendedBook(book_stub)
    ...
    local function attemptBookDetails()
        ...
        local task = function()
            return Api.getBookDetails(user_session and user_session.user_id, user_session and user_session.user_key,
        ...
        local on_success = function(api_result)
            ...
            Ui.closeMessage(loading_msg)
            logger.info(string.format("Zlibrary:onSelectRecommendedBook - Fetch successful for book ID: %s",
                api_result.book.id))
...
function Zlibrary:performSearch(query)
    ...
    local function attemptSearch(retry_on_auth_error)
        ...
        local task = function()
            return Api.search(query, user_session and user_session.user_id, user_session and user_session.user_key,
                selected_languages, selected_extensions, selected_order, current_page_to_search)
        end
        ...
        on_success = function(api_result)
            if api_result.error then
                ...
                Ui.showSearchErrorDialog(api_result.error, query, user_session, selected_languages, selected_extensions,
                    selected_order, current_page_to_search, loading_msg, on_success, function(final_err_msg)
                        -- Cancel callback - user already knows about the error
                    end)
                return
            end
        ...
        local on_error_handler = function(err_msg)
            ...
            -- Use the retry dialog for timeouts and HTTP 400 errors
            Ui.showSearchErrorDialog(err_msg, query, user_session, selected_languages, selected_extensions,
                selected_order, current_page_to_search, loading_msg, on_success, function(final_err_msg)
                    -- Cancel callback - user already knows about the error
                end)
        end
...
function Zlibrary:displaySearchResults(initial_book_data_list, query_string)
    ...
    local menu_items = {}
    logger.info(string.format("Zlibrary:displaySearchResults - Preparing menu items from %d initial results.",
        #initial_book_data_list))
    ...
    local function on_goto_page_handler(menu_instance, new_page_number)
        ...
        if is_last_page_of_current_items and self.has_more_api_results then
            logger.info(string.format(
                "Zlibrary: Reached page %d (last page of current items). Attempting to load more from API.",
                new_page_number))

            local next_api_page_to_fetch = self.current_search_api_page_loaded + 1
            local loading_msg_more = Ui.showLoadingMessage(string.format(T("Loading more results (Page %s)..."),
                next_api_page_to_fetch))
            ...
            local task_load_more = function()
                return Api.search(self.current_search_query, user_session_more.user_id, user_session_more.user_key,
                    selected_languages_more, selected_extensions_more, selected_order_more, next_api_page_to_fetch)
            end
            ...
            on_success_load_more = function(api_result_more)
                ...
                if api_result_more.error then
                    ...
                    Ui.showErrorMessage(
                        Ui.colonConcat(T("Failed to load more results"), tostring(api_result_more.error)))
                    return
                end
...
function Zlibrary:downloadBook(book)
    ...
    if not target_dir then
        ...
        logger.warn(string.format("Zlibrary:downloadBook - Download directory setting not found, using fallback: %s",
            target_dir))
    else
    ...
    local function attemptDownload(retry_on_auth_error)
        ...
        local function on_success_download(api_result)
            ...
            if api_result and api_result.success then
                ...
                end, function(should_turn_off_wifi)
                    if should_turn_off_wifi then
                        NetworkMgr:disableWifi(function()
                            logger.info("Zlibrary:downloadBook - Wi-Fi disabled after download as requested by user")
                        end)
                ...
                if api_result and api_result.error and
                    string.find(api_result.error, "Download limit reached or file is an HTML page", 1, true) then
...
function Zlibrary:onCloseWidget()
    if self.dialog_manager and self.dialog_manager:getDialogCount() > 0 then
        logger.info("Zlibrary:onCloseWidget - Cleaning up " .. self.dialog_manager:getDialogCount() ..
                        " remaining dialogs")
        self.dialog_manager:closeAllDialogs()
    end
end
